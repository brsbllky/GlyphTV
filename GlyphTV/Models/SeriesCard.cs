...
namespace GlyphTV
{
    public class SeriesCard : INotifyPropertyChanged
    {
        ...
        // ── YENİ: Dizi sıralaması için eklenen alanlar ──
        public DateTime AddedDate { get; set; } = DateTime.MinValue;
        public long LatestStreamId { get; set; } = 0;
        ...
        public string EpisodeText =>
            CurrentEpisodes.Count > 0 && _selectedEpisodeIndex < CurrentEpisodes.Count
                ? $" B{CurrentEpisodes[_selectedEpisodeIndex].EpisodeNumber:D2}" : "-";
        ...
        {
            get => EpisodesBySeason.Values.Any(list => list.Any(ep => ep.IsFavorite));
            set
            {
                foreach (var list in EpisodesBySeason.Values)
                {
